...
const ReplyInput = z.object({
  threadId: z.string().min(1).describe("Existing Gmail thread id; never create a new thread."),
  to: z.string().email(),
  subject: z.string().max(200),
  inboundMessageId: z.string().min(1).describe("Message id of the latest inbound message."),
...
export default defineTool({
  ...
  async execute(input) {
    if (input.doNotContact) {
      return { status: "blocked", reason: "CONTACT_SUPPRESSED", threadId: input.threadId };
    }
    const received = Date.parse(input.inboundReceivedAt);
    if (!Number.isFinite(received) || Date.now() - received > 30 * 24 * 60 * 60 * 1000) {
      return { status: "blocked", reason: "INBOUND_CONTEXT_EXPIRED", threadId: input.threadId };
    }
    return {
      status: "draft_ready",
      channel: "EMAIL",
      ...
      audit: { createdAt: new Date().toISOString(), source: "EVE_GMAIL_REPLY_DRAFT" },
    };
  },
});
