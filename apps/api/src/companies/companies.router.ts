...
export class CompaniesRouter {
	constructor(
		...
		@Inject(LeadReviewService) private readonly leadReview: LeadReviewService,
	) {}
	...
	@Mutation({ input: companyLeadReviewInput })
	async reviewLead(
		@Ctx() ctx: AuthedTrpcContext,
		@Input() input: z.infer<typeof companyLeadReviewInput>,
	) {
		return this.leadReview.decide(
			input.id,
			input.decision,
			ctx.user.name,
			input.reason,
