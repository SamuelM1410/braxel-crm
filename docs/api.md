...
Onboarding, and onboarding only. Asked server-side every request.

- **Braxel does not gate on the Context key.** Upstream asked for it at
  `/onboarding/research`; this fork replaced that page with a redirect to the app, so
  reading a research gate in the proxy sends a rep into a redirect loop — the page
  bounces to `/`, and the proxy bounces `/` back to the page. The key lives on
  Settings → General, and a missing one removes research instead of blocking the CRM.
  `research-form.tsx` is the upstream form, kept but unrouted.
...
- **A keyless install is not stranded, but it is not researching either** — companies
  sit `PENDING` until a key is saved on Settings → General. Nothing in the UI says so
  today.
